...
namespace Events
{
    ...
    {
        private static readonly Dictionary<Type, Event> _eventTypeDictionary = new();

        public static void AddListener<T>(Action<object> action) where T : Event, new()
        {
            var type = typeof(T);
            var hasType = _eventTypeDictionary.TryGetValue(type, out Event evt);

            if (!hasType)
            {
                evt = new T();
                _eventTypeDictionary.Add(type, evt);
            }
        ...
        {
            var hasType = _eventTypeDictionary.TryGetValue(typeof(T), out Event evt);
